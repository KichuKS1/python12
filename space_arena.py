...
wn.title("Space Arena! BY Devika and Team")
wn.bgcolor("black")
...
character_pen.draw_string(pen, "BY DEVIKA AND TEAM", 0, 100)
...
for enemy in enemies:
    x = random.uniform(-game.width/2.0, game.width/2.0)
    y = random.uniform(-game.height/2.0, game.height/2.0)
...
stars = []    
for _ in range(20):
    x = random.uniform(int(-game.width/4.0), int(game.width/4.0))
    y = random.uniform(int(-game.height/4.0), int(game.height/4.0))
